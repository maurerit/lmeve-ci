 ...
 */
class Auth {
    public function check_includes_for_auth_php() {
            return TRUE;
    }
    ...
    public function check_changed_session_ip() {
    ...
    public function check_changed_session_path() {
    ...
    public function checkrights($rightNames) { //comma-separated string
            //check if user has at least one of the comma-separated list of rights
            if (!Auth::userexists()) return FALSE;
            $tmp="";
    ...
    public function getusername() { //obsolete and stupid!!
            //if (!userexists()) return FALSE;
            global $USERSTABLE;
            if (isset($_SESSION['granted'])) {
    ...
    public function updatelast($date,$ip) {
            if (!userexists()) return FALSE;
            global $USERSTABLE;
            if (isset($_SESSION['granted'])) {		
                            $sql="UPDATE `$USERSTABLE` SET
    ...
    public function hashpass($pass) { //create a salted hash
            global $LM_SALT;
